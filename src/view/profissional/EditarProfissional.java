...
public class EditarProfissional extends javax.swing.JFrame {
    ...
    private void edit() {
        ...
        if(Validacao.loginExists(tLogin.getText(), this.profissional.getId())) {
        ...
        if(!Validacao.isEmpty(CEP) || !Validacao.isEmpty(tRua.getText())) {
            ...
            if(this.profissional.getEndereco() != null)
                e.setId(this.profissional.getEndereco().getId());
            else
                e.setId(0);
        }
        ...
        if(Validacao.isEmpty(new String(tSenha.getPassword())))
            u.setSenha(this.profissional.getUsuario().getSenha());
        else
            u.setSenha(md5.md5(new String(tSenha.getPassword())));
        u.setId(this.profissional.getUsuario().getId());
        ...
        prof.setId(this.profissional.getId());
        
        ProfissionalController pc = new ProfissionalController();
        if(pc.editar(prof)){
            List<Profissional> lista = pc.profissionais();
            if(lista!=null)
